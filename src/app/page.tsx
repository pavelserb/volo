...
export default function HomePage() {
  ...
  const heliSku = localizeSku(skus.find((s) => s.directionSlug === 'helicopters')!, locale);
  const transferSku = localizeSku(skus.find((s) => s.directionSlug === 'transfers')!, locale);
  const featuredReview = heliSku.reviews[0];

  const whyItems = [
    { icon: <Shield size={28} strokeWidth={1.5} />, titleKey: 'common.why1Title' as const, textKey: 'common.why1Text' as const },
    { icon: <Clock size={28} strokeWidth={1.5} />, titleKey: 'common.why2Title' as const, textKey: 'common.why2Text' as const },
    { icon: <Gift size={28} strokeWidth={1.5} />, titleKey: 'common.why3Title' as const, textKey: 'common.why3Text' as const },
  ];
        ...
        <div className="container-wide section-padding mx-auto py-8 sm:py-10">
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-8 text-center">
            {[
              ...
              { value: '100%', labelKey: 'common.insured' as const, suffix: '' },
              ...
              <div className="relative aspect-[4/3] rounded-2xl overflow-hidden shadow-volo-xl">
                ...
                <div className="absolute bottom-4 left-4 flex items-center gap-1.5 bg-black/50 backdrop-blur-sm rounded-full px-3 py-1.5">
                  ...
                  <span className="text-white/60 text-xs">
                    {t('common.reviewsCount', { count: String(heliSku.reviews.length) })}
                  </span>
                </div>
              </div>
            </div>
            ...
            <div className="lg:bg-volo-surface lg:rounded-2xl lg:p-10 xl:p-14 lg:shadow-volo-lg lg:ml-0 lg:-ml-4 relative">
              ...
              <div className="mt-6 flex flex-wrap gap-3">
                <span className="inline-flex items-center gap-1.5 rounded-full bg-volo-bg px-3.5 py-1.5 text-sm text-volo-text border border-volo-border">
                  <Clock size={14} className="text-volo-accent" /> {heliSku.durationMinutes} {t('common.min')}
                </span>
                <span className="inline-flex items-center gap-1.5 rounded-full bg-volo-bg px-3.5 py-1.5 text-sm text-volo-text border border-volo-border">
                  <Users size={14} className="text-volo-accent" /> {t('common.upTo')} {heliSku.capacityTotal}{' '}
                  {t('common.people')}
                </span>
              </div>
          ...
          <div className="text-center max-w-2xl mx-auto">
            ...
            <div className="mt-6 flex items-center justify-center gap-3">
              ...
              <div className="text-left">
                ...
                <p className="text-xs text-white/40">{t('common.scenicFlight')}</p>
              </div>
            </div>
          </div>
        </div>
            ...
            <div className="lg:bg-volo-bg lg:rounded-2xl lg:p-10 xl:p-14 lg:shadow-volo-lg lg:mr-0 lg:-mr-4 relative order-2 lg:order-1">
              ...
              <div className="mt-6 flex flex-wrap gap-3">
                <span className="inline-flex items-center gap-1.5 rounded-full bg-volo-surface px-3.5 py-1.5 text-sm text-volo-text border border-volo-border">
                  <Clock size={14} className="text-volo-accent" /> {t('common.approx')} {transferSku.durationMinutes}{' '}
                  {t('common.min')}
                </span>
                <span className="inline-flex items-center gap-1.5 rounded-full bg-volo-surface px-3.5 py-1.5 text-sm text-volo-text border border-volo-border">
                  <Users size={14} className="text-volo-accent" /> {t('common.upTo')} 4 {t('common.passengers')}
                </span>
              </div>
            ...
            <div className="relative lg:-ml-8 z-10 order-1 lg:order-2">
              <div className="relative aspect-[4/3] rounded-2xl overflow-hidden shadow-volo-xl">
                ...
                <div className="absolute bottom-4 left-4 flex items-center gap-1.5 bg-black/50 backdrop-blur-sm rounded-full px-3 py-1.5">
                  ...
                  <span className="text-white/60 text-xs">
                    {t('common.reviewsCount', { count: String(transferSku.reviews.length) })}
                  </span>
                </div>
              </div>
            </div>
      ...
      <section className="bg-volo-bg border-y border-volo-border">
        <div className="container-wide section-padding mx-auto py-16 sm:py-20 lg:py-24">
          ...
          <div className="grid sm:grid-cols-3 gap-6 lg:gap-10">
            {whyItems.map((item) => (
              <div key={item.titleKey} className="text-center group">
                ...
                <h3 className="font-heading text-lg font-semibold text-volo-text mb-2">{t(item.titleKey)}</h3>
                <p className="text-sm text-volo-muted leading-relaxed max-w-xs mx-auto">{t(item.textKey)}</p>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="relative z-10 section-padding container-wide w-full">
          <div className="max-w-xl">
            ...
            <h2 className="text-3xl sm:text-4xl lg:text-5xl font-heading font-bold text-white leading-tight">
              {t('common.gift')}
              <br />
              <span className="text-volo-accent">{t('common.giftAccent')}</span>
            </h2>
